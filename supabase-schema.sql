...
-- ── [MỚI] Dữ liệu đồng bộ từ phuongthaovip-main / bot_v23.py ────────────────
-- Lưu nguyên khối JSON donhang_by_subid / vitien_by_subid / da_nhan_by_subid,
-- key My ID (= sub_id bên bot) là khoá trong JSON, không tách bảng quan hệ để
-- luôn khớp 100% với dữ liệu bot đang dùng, tránh lệch schema khi bot đổi format.
...
-- dùng nữa — đơn hàng & ví tiền giờ tính trực tiếp từ bot_data theo My ID,
-- giống hệt cách bot_v23.py tính cho lệnh #donhang / #vitien. Có thể giữ lại
-- bảng orders/cột wallet_balance để tương thích ngược, không bắt buộc xoá.
