 ...
 *   co_the_rut = tổng (hoa_hong * 89% * 80%) của đơn hoàn thành >= 1 ngày
 *   hoan_thanh_chua_rut = tổng (hoa_hong * 89% * 80%) của đơn hoàn thành < 1 ngày
...
export function calcVitien(vitienData, daNhanData, subId) {
  ...
  for (const don of v.don_hoan_thanh || []) {
    ...
    const netAmount = Math.round(hh * 0.89 * 0.8 * 100) / 100;
...
// Chiết khấu 1 khoản tiền gốc -> trừ thuế 11% -> còn 80% thực nhận.
...
/**
 * Tổng số tiền đã quy đổi (đã trừ 11% thuế + nhân 80%) của 1 sub_id — dùng
...
// Chiết khấu 1 đơn: hoa hồng gốc -> trừ thuế 11% -> còn 80% hoa hồng thực nhận.
...
// sub_id "dạng số đủ 18 chữ số" (vd Zalo ID dài như 155624444817411152) —
// dùng để lọc ở trang Admin, bỏ qua các sub_id là tên gợi nhớ (chữ) hoặc
// số nhưng không đủ 18 chữ số.
export function isNumericSubId18(subId) {
  return /^\d{18}$/.test(String(subId ?? "").trim());
}

/**
 * Danh sách TOÀN BỘ đơn hàng của mọi sub_id dạng số đủ 18 chữ số — dùng cho
 ...
 * thuế -11%/80%, trạng thái, ngày đặt/hoàn thành. Sắp mới nhất trước.
 */
