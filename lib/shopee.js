...
const AFFILIATE_ID = "17334820176"; // ID affiliate Shopee của bạn
...
function formatVnd(amount) {
  if (!amount) return "—";
  return Math.trunc(amount).toLocaleString("vi-VN") + "đ"; // cắt thập phân, không làm tròn lên
}
...
async function fetchProductInfo(shopeeUrl) {
  try {
    ...
    // ── Tính hoa hồng — LOGIC GIỐNG BOT (bot_v23.py._fetch_commission) ──────
    const price = p.price || 0;
    let sellerCom = p.sellerComFinal || 0; // hoa hồng Xtra (seller) — giữ nguyên
    const shopeeComRaw = p.shopeeComFinal || 0; // hoa hồng Shopee do API trả về

    const NEW_RATE = 0.08; // mức mới: 8%
    const NEW_CAP = 50000; // cap tối đa 50k

    // Ép lên 8% nếu Shopee trả về LỚN HƠN 0% (chỉ cần > 0%)
    const isOldScheme = price ? shopeeComRaw / price > 0 : false;

    let shopeeCom, shopeeRate;
    if (isOldScheme) {
      shopeeCom = Math.min(price * NEW_RATE, NEW_CAP); // tính lại đúng 8%, cap 50k
      shopeeRate = NEW_RATE; // % hiển thị luôn là 8% danh nghĩa
    } else {
      shopeeCom = shopeeComRaw; // không khớp mức cũ → giữ nguyên như API trả về
      shopeeRate = price ? shopeeComRaw / price : 0;
    }

    sellerCom = Math.max(sellerCom, 0); // Xtra âm → coi như 0, không trừ vào tổng
    const sellerRate = price ? sellerCom / price : 0;

    const commission = sellerCom + shopeeCom;
    const commissionPct = `${((shopeeRate + sellerRate) * 100).toFixed(1)}%`;
    // ──────────────────────────────────────────────────────────────────────
